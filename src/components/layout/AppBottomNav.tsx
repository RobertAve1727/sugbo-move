...
const AppBottomNav = ({
  ...
  return (
    <nav className="sticky bottom-0 w-full z-50 flex justify-around items-center px-4 pb-6 pt-3 bg-white shadow-[0_-4px_24px_rgba(0,13,34,0.06)] border-t border-[#e6e8ea]/15 rounded-t-2xl">
      <button type="button" onClick={onExplore} className={getTabClass("explore")}>
        <span className="material-symbols-outlined">explore</span>
        <span className="font-[Inter] text-[10px] font-bold uppercase tracking-widest mt-1">Explore</span>
      </button>

      <button type="button" onClick={onRoutes} className={getTabClass("routes")}>
        <span className="material-symbols-outlined">directions_car</span>
        <span className="font-[Inter] text-[10px] font-bold uppercase tracking-widest mt-1">Routes</span>
      </button>

      <button type="button" onClick={onSavings} className={getTabClass("savings")}>
        <span className="material-symbols-outlined">payments</span>
        <span className="font-[Inter] text-[10px] font-bold uppercase tracking-widest mt-1">Savings</span>
      </button>

      <button type="button" onClick={onProfile} className={getTabClass("profile")}>
